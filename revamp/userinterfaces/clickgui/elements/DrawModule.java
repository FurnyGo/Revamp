...
public class DrawModule
{
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        Gui.drawRect(this.x, this.y, this.x + 100, this.y + this.height, -1157627904);
        if (this.isHovered(mouseX, mouseY)) {
            Gui.drawRect(this.x, this.y, this.x + 100, this.y + this.height, 0);
        }
        final int textColor = this.module.isToggled() ? (Element.getSetting(Element.moduleName("ClickGUI"), "Rainbow").isToggled() ? Rainbow.rainbow(4.0f, 0.5f, 1.0f, 200L) : Revamp.getClientCOLOR().getRGB()) : new Color(-1).getRGB();
        this.fr.drawStringWithShadow(this.module.getName(), (float)(this.x + 50 - this.fr.getStringWidth(this.module.getName()) / 2), this.y + this.height / 2 - this.fr.FONT_HEIGHT / 2 + 1, textColor);
        int settingWidth = 0;
        ...
        for (final Element element2 : this.elements) {
            if (element2.setting.isVisible() && this.extended) {
                element2.updatePosition(this.x + this.width, y, settingWidth, this.height);
