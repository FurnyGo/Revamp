...
public class Aura extends Module implements Methods
{
    ...
    public void onEvent(final Event event) {
        ...
        if (event instanceof EventPre) {
            ...
            if (!this.noswing.isToggled() && this.autoblock.isToggled() && this.target.getDistanceToEntity(this.getPlayer()) < this.reach.getCurrentValue() && !this.blockmode.getCurrentMode().equalsIgnoreCase("Fake")) {
                this.blockMode();
            }
    ...
    private void blockMode() {
        ...
        else if (this.blockmode.getCurrentMode().equalsIgnoreCase("AAC") && this.getPlayer().ticksExisted % 2 == 0) {
            this.getPlayerController().sendUseItem(this.getPlayer(), this.getWorld(), this.getPlayer().inventory.getCurrentItem());
        }
    }
    
    public boolean fakeAutoblockEnabled() {
        return this.autoblock.isToggled() && this.getPlayer().inventory.getCurrentItem() != null && this.blockmode.getCurrentMode().equalsIgnoreCase("Fake") && this.getPlayer().inventory.getCurrentItem().getItem() instanceof ItemSword && this.target != null;
    }
