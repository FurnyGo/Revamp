...
public class InventoryManager extends Module implements Methods
{
    ...
    public InventoryManager() {
        ...
        this.junk = Arrays.asList("stick", "egg", "string", "cake", "mushroom", "flint", "dyePowder", "feather", "chest", "snowball", "fish", "enchant", "exp", "shears", "anvil", "torch", "seeds", "leather", "reeds", "skull", "record", "piston", "snow", "poison", "tnt", "sugar");
    ...
    public boolean purgeJunk() {
        for (int i = 9; i < 45; ++i) {
            ...
            if (stack != null) {
                final Item item = stack.getItem();
                for (final String shortName : this.junk) {
                    if (!stack.getDisplayName().toLowerCase().contains("(right click)") && item.getUnlocalizedName().contains(shortName) && this.timer.hasReached((long)this.delay.getCurrentValue())) {
                        this.purge(i);
                        return false;
                    }
                }
